...
class App.Model.Application
  ...
  set: (value) -> @transaction =>
    ...
    @root._changes.push [ @selectorHistory.join(':'), 'set', value ]
  ...
  push: (value) -> @transaction =>  
    @root._needsSave = true
    @root._changes.push [ @selectorHistory.join(':'), 'push', value ]
  ...
  remove: (iterator) -> @transaction =>
    @root._needsSave = true
    @root._changes.push [ @selectorHistory.join(':'), 'remove', iterator ]
  ...
  transaction: (callback) ->
    ...
    delayed = false
    if changeState
      delayedCallback = => if !delayed then delayed = true else @root._transaction = false; @_trigger()
    else delayedCallback = ->

    ret = callback(delayedCallback)

    # If this is the outermost transaction, commit.
    if changeState && !delayed
  ...
  trigger: (action) ->
    ...
    if !@root._transaction then @_trigger()
  ...
  clone: -> new @__type @_clone()
  _clone: ->
    switch @dataModel.type
      when 'object'
        ret = {}
        for k, v of @dataModel.items
          ret[k] = @_(k)._clone()
        ret
      when 'list'
        ret = []
      ...
      when 'keyedList'
        ret = []
        for v, k in @data
          ret.push @_(v.id)._clone()
        ret

      when 'value'
        @data

  url: -> '/api/v0/applications' + (if !@root.isNew? then '/' + @root.get('id'))
  save: (callback) -> @transaction =>
    Backbone.sync (if @root._isNew? then 'create' else 'update'), @root, { success: callback }
    @root._needsSave = false
    @root.trigger 'save'
  getUsers: (callback) ->
    $.getJSON '/api/v0/applications/' + @root.get('id') + '/users', callback
